function getCase(node,id){
    $(node).addClass('active').siblings().removeClass('active');
    $.ajax({
        url:(compassIntroduceIsLocalHost?'/wumartCompass/mokeData/':compassIntroduceOrigin+'/') + 'vendor/compass/notice' + (compassIntroduceIsLocalHost ? '.json' : ''),
        type:'get',
        dataType:'json',
        beforeSend:function(){
            compassIntroduceLoading= new compassIntroduceLoadingNode();
        },
        complete:function(){
            compassIntroduceLoading.hide();
        },
        data:{
            id:id
        },
        success:function(data){
            if(data){
                ...
                $('.compass-introduce-right h4').html(data.title+'<small>'+newTime+'</small>');
                $('.compass-introduce-right p').html(data.context);
            }else{
...
/*页面开始加载动画*/
var compassIntroduceLoadingNode = function () {
    var that = this;
    that.id = '';
...
compassIntroduceLoadingNode.prototype = {
    show: function () {
        var that = this;
...
var compassIntroduceLoading;
var compassIntroduceHref=location.href;
var compassIntroduceId=compassIntroduceHref.split('=')[1];
var compassIntroduceIsLocalHost = location.hostname.toLowerCase() == 'localhost';
var compassIntroduceOrigin=window.location.protocol + "//" + window.location.hostname;
$(function(){
    compassIntroduceLoading= new compassIntroduceLoadingNode();
    $.when(
        $.ajax({
            url:(compassIntroduceIsLocalHost?'/wumartCompass/mokeData/':compassIntroduceOrigin+'/') + 'vendor/compass/bigdata' + (compassIntroduceIsLocalHost ? '.json' : ''),
            type:'get',
            dataType:'json',
            data:{}
        }),
        ...
        for(var i=0;i<data1.length;i++){
            list+='' +
                '<li onclick="getCase(this,'+data1[i].id+');">' +
                    '<div class="compass-introduce-title">' +
                        '<div class="pic-index">'+(i+1)+'</div>' +
                        '<span>'+data1[i].title+'</span>' +
                    '</div>' +
                '<div class="ellipsis">'+data1[i].context+'</div>' +
                '</li>';
        }
        $('.compass-introduce-left ul').html(list).find('li').eq(0).addClass('active');
        //超出文本省略号
        $(".ellipsis").each(function(){
            var $p = $(this);
            while ($p.height() > 50) {  //50是自己根据两行文字高度确定的
                $p.text($p.text().replace(/(\s)*([a-zA-Z0-9]+|\W)(\.\.\.)?$/, "..."));
            }
        });
        /*右边*/
        var publishTime=new Date(data2.publishTime);
        var newTime=publishTime.getFullYear()+'-'+((publishTime.getMonth()+1)<9?('0'+(publishTime.getMonth()+1)):(publishTime.getMonth()+1))+'-'+publishTime.getDate();
        $('.compass-introduce-right h4').html(data2.title+'<small>'+newTime+'</small>');
        $('.compass-introduce-right p').html(data2.context);
        compassIntroduceLoading.hide();
    }).fail(function(){
        compassIntroduceLoading.hide();
        window.wxc.xcConfirm('服务器错误', window.wxc.xcConfirm.typeEnum.error);
